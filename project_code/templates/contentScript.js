...
function print_res_of_kwd(index)
        ...
        //    alert(video_substring);					
			...
			if(response.final_results[index].books != " ")
			{
				...
				{	
					...
					str=response.final_results[index].books[i].link;
					//alert(str);
					str = str.replace(/'/g,"");       //to replace quotes in string with blank string..working properly
					book_markup += "<li><a href="+str+">"+response.final_results[index].books[i].title+"</a><br />"+response.final_results[index].books[i].snippet+"</li><hr />";
				}
				book_markup += "</ul>"
						
				$(clss).append(book_markup);
			}
			//Location Specific Websites
			if(response.final_results[index].location != " ")
			{
			
				var location_markup = "<ul>",clss;
				for(i = 0; i<3 ; i++)
				{	
				
					str=response.final_results[index].location[i].link;
					//alert(str);
					str = str.replace(/'/g,"");       //to replace quotes in string with blank string..working properly
					location_markup += "<li><a href="+str+">"+response.final_results[index].location[i].title+"</a><br />"+response.final_results[index].location[i].snippet+"</li><hr />";
				}
				location_markup += "</ul>"
						
				$(clss).append(location_markup);
			}
